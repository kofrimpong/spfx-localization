...
const subscriptionKey = process.env.TRANSLATION_API_KEY;
...
/**
 * 
 * @param {string} relativeLocalFolder 
 * @param {array} locales array of ICU Local in lower cases
 */
const run = async (relativeLocalFolder,locales) => {
  // Load files in the localization directory
  const locDirPath = path.join(process.cwd(), relativeLocalFolder);
  let locFiles = fs.readdirSync(locDirPath);
  locFiles = locFiles.filter(f => f !== "mystrings.d.ts" && f != "en-us.js");

  // Load main localization file
  const mainLoc = locHelper.getSPLocalizationFileAsJSON(locDirPath,'en-us');

  // Iterate over all supported languages and prepare translation request
  let langs = locales ? locales : languagesConfiguration.langs;
  for (const lang of langs) {
    console.log(`Processing ${lang}.`);

    // If current loc file doesn't exist - copy the original one
    let currentLoc = locHelper.getSPLocalizationFileAsJSON(locDirPath,lang);
    if (!currentLoc) {
    ...
    if (translatedObj) {
      locHelper.replaceTranslatedKeysInJSON(locDirPath,translatedObj, lang)
    }
